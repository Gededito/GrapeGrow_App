...
class _MapSebaranHamaState extends State<MapSebaranHama> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Stack(
          children: [
            BlocBuilder<AllSebaranHamaBloc, AllSebaranHamaState>(
              builder: (context, state) {
                return state.maybeWhen(
                  orElse: () => const Center(child: Text("Lokasi Tidak Muncul")),
                  loading: () => const Center(child: CircularProgressIndicator()),
                  success: (datas) {
                    addManyMarker(datas.sebaranHama);

                    if (myLocation != null) {
                      final allLocations = [myLocation!, ...datas.sebaranHama.map((sebaran) => LatLng(sebaran.lat, sebaran.lon))];
  ...
  void addManyMarker(List<SebaranHama> sebaranList) async {
