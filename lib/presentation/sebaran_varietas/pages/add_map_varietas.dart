...
class AddMapVarietas extends StatefulWidget {
  final double lat;
  final double lon;

  const AddMapVarietas({
    super.key,
    required this.lat,
    required this.lon,
  });
  ...
  @override
  State<AddMapVarietas> createState() => _AddMapVarietasState();
}

class _AddMapVarietasState extends State<AddMapVarietas> {
  ...
  Future<void> _getPosition({
    ...
    if (mounted) {
      context.read<AddMapVarietasBloc>().add(
        AddMapVarietasEvent.setValue(
          data: MapModel(
